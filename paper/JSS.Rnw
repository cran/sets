...
\SweaveOpts{engine=R,eps=TRUE,height=6,width=7,results=verbatim,fig=FALSE,echo=TRUE}
...
\begin{figure}[h]
\begin{center}
<<clusterplot,fig=TRUE,echo=FALSE>>=
plot(cl1)
@
...
<<>>=
tdm <- set_outer(c("neural networks", "fuzzy", "image"), fuzzy_docs, `%in%`)
print(tdm)
@
...
\begin{figure}[h]
\begin{center}
<<clusterplot2,fig=TRUE,echo=FALSE>>=
plot(cl2)
@
